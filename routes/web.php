...
Route::middleware(['auth'])->group(function () {
    ...
    Route::middleware(['ensure.admin.access'])->group(function () {
        ...
        Volt::route('employees/inactivos', 'employees.inactivos')->name('employees.inactivos');

        // Users management routes
        Volt::route('users', 'users.index')->name('users.index');
        Volt::route('users/{id}/assign-roles', 'users.assign-roles')->name('users.assign-roles');
        Route::get('users/download-template', [UserController::class, 'downloadCategoryTemplate'])->name('users.download-template');
